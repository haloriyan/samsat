...
@section('title', "Form Permohonan Menjadi Wajib Pajak Air Permukaan")
...
@section('content')
    ...
    @if ($datas->count() == 0)
        <h2>Form Permohonan Menjadi Wajib Pajak Air Permukaan</h2>
        <form action="{{ route('app.pap.store') }}" method="POST">
            {{ csrf_field() }}
            ...
            <p>adalah benar dan ingin mengirimkan permohonan untuk menjadi Wajib Pajak Air Permukaan</p>

            <button class="mt-3 biru lebar-100">Submit</button>
        </form>
    @else
        <h3>
        Maaf, Anda tidak bisa mengirim permohonan menjadi Wajib Pajak Air Permukaan karena
        @if ($datas[0]->status == 1)
